...
Looks pretty good: clusters from CGF and MLST are generally smaller, and often show much higher concordance to the underlying epidemiology. 

Couple points to consider: 


1) Some isolates are forced into singleton clusters when assessed via CGF or MLST therefore not included in the analysis. 

```{r}

typing.data_ns <- subset(typing.data, duplicated(CC) | duplicated(CC, fromLast = T))

typing.data_ns <- subset(typing.data_ns, duplicated(ST) | duplicated(ST, fromLast = T))

typing.data_ns <- subset(typing.data_ns, duplicated(CGF) | duplicated(CGF, fromLast = T))
...
Now lets try the same thing for MLST Results: 
